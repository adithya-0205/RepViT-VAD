...
ARSON_SRC = "Arson"
EXTRACTED_DIR = "extracted_frames/train/arson"
DATASET_TRAIN = "dataset/train/arson"
DATASET_VAL = "dataset/val/arson"
...
if __name__ == "__main__":
    if not os.path.exists(ARSON_SRC):
        print(f"Error: {ARSON_SRC} does not exist.")
        exit(1)

    videos = sorted([
        f for f in os.listdir(ARSON_SRC)
        if f.lower().endswith((".mp4", ".avi", ".mov", ".mkv"))
    ])

    print(f"Found {len(videos)} Arson videos. Extracting frames...")

    for v in videos:
        v_name = os.path.splitext(v)[0]
        v_path = os.path.join(ARSON_SRC, v)
        ...
        if not os.path.exists(out_path):
            ...
            print(f"Frames already extracted for {v}")
    ...
    print(f"\nSuccessfully prepared Arson dataset: {len(train_vids)} train videos, {len(val_vids)} val videos!")
